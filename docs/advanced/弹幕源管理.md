...
![资源仓库与搜索源](/screenshot/资源仓库与搜索源.png)
   ...
   **官方仓库:**
   ...
   - Gitee 仓库链接:
   ```
   https://gitee.com/l429609201/Misaka-Scraper-Resources
   ```
   >PS:Gitee 仓库需要弹幕库版本大于v2.3.9，主仓库更新后，Github往Gitee推送大概需要5分钟左右，但是大陆环境网络下，连通性比Github好，推荐连接Github不太稳定的用户使用。
...
::: tip v2.7.4 弹幕源体验优化
v2.7.4 起，弹幕源管理页做了两项更友好的改动：

- 弹幕源支持 `displayName` / `display_name` 友好显示名称，前端会优先显示更易读的名称
- “添加数据源”弹窗中的平台列表改为后端动态获取，不再依赖前端硬编码列表
:::
...
1. 访问 [Github](https://github.com/l429609201/Misaka-Scraper-Resources/releases) / [Gitee](https://gitee.com/l429609201/Misaka-Scraper-Resources/releases)  资源仓库 Releases 页面
2. 下载对应平台和架构的离线包（如 `linux-x86.zip`、`windows-amd64.zip` 等）
