...
本服务兼容 [弹弹play API](https://api.dandanplay.net/swagger/index.html) 规范，提供以下核心接口：
...
**标签说明**：
- 标签范围：`#A`、`#B`、`#C`、`#D`、`#E`（对应最近播放的5部番剧）
- 集数编号：从 1 开始，如 `#A1` 表示第 1 集，`#A10` 表示第 10 集
...
::: tip 导航栏流控指示器
v2.7.2 起，Web UI 导航栏会显示下载流控与后备流控状态，方便直接观察当前是否处于限流或排队状态。

如果需要在播放器侧查询详细状态，仍然可以使用 `@CXLK` 指令。
:::
...
Token 管理中可以查看单个 Token 的访问日志，用于排查客户端调用情况。

v2.7.2 起，访问日志增强记录了更多信息：
...
::: tip MCP 调用日志
通过 MCP Server 发起的调用也会进入外部控制/Token 相关日志，并带有 `MCP:` 前缀，方便区分普通外部控制调用和 MCP 工具调用。
:::
...
- v2.7.0 起支持 **填充默认配置** 按钮，可一键写入推荐黑名单规则
...
::: tip 默认配置说明
如果您不想从零开始编写规则，可以直接使用 **填充默认配置** 按钮导入推荐规则，再根据自己的观影习惯做微调。
:::
